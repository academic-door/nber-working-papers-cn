(function () {
  ...
  function prefetchWeeklyYear(year) {
    const selectedYear = String(year || weeklyManifest?.latest_year || "");
    if (!selectedYear || weeklyYearCache.has(selectedYear) || weeklyYearPromises.has(selectedYear)) return;
    const promise = loadJson(`data/search/weekly/${selectedYear}.json`)
      .then((rows) => {
        weeklyYearCache.set(selectedYear, rows || []);
        return rows || [];
      })
      .catch((error) => {
        weeklyYearPromises.delete(selectedYear);
        throw error;
      });
    weeklyYearPromises.set(selectedYear, promise);
  }

  function scheduleLatestWeeklyPrefetch() {
    const latestYear = String(weeklyManifest?.latest_year || "");
    if (!latestYear) return;
    const task = () => prefetchWeeklyYear(latestYear);
  ...
  async function ensureWeeklyPapers(year) {
    ...
    if (weeklyYearCache.has(selectedYear)) {
      weeklyPapers = weeklyYearCache.get(selectedYear) || [];
      return;
    }
    paperList.innerHTML = `<p class="loading-state">${selectedYear} 年周报索引加载中...</p>`;
    if (!weeklyYearPromises.has(selectedYear)) {
      prefetchWeeklyYear(selectedYear);
    }
  ...
  function renderPapers() {
    ...
    if (!filtered.length) {
      const scopeLabel = sourceMode === "weekly" ? "周报索引" : "月度中文合集";
  ...
  function updateActiveFilters(query, year) {
    ...
    const sourceLabel = sourceMode === "weekly" ? "周报索引" : "月度中文合集";
    const effectiveYear = sourceMode === "weekly"
      ? String(year || weeklyManifest?.latest_year || "")
      : String(year || "");
    const relationLabel = {
      all: "全部论文",
      china: "中国相关",
    ...
    const parts = [sourceLabel, effectiveYear ? `${effectiveYear} 年` : "全部年份", relationLabel];
    ...
    if (scopeContext) {
      if (sourceMode === "weekly") {
        scopeContext.textContent = `当前统计范围：周报索引 · ${effectiveYear || "最新年份"}`;
      } else if (effectiveYear) {
        scopeContext.textContent = `当前统计范围：月度中文合集 · ${effectiveYear} 年`;
      } else {
        const years = (monthlyManifest?.years || []).map((item) => Number(item.year)).filter(Number.isFinite);
        const range = years.length ? `${Math.min(...years)}–${Math.max(...years)}` : "全部年份";
        scopeContext.textContent = `当前统计范围：月度中文合集 · ${range}`;
      }
    }
  }
  ...
  async function setSourceMode(value) {
    ...
    if (sourceMode === "weekly") {
      ...
      yearFilter.options[0].textContent = "最新年份";
  ...
  loadIndexData()
    .then(() => {
      ...
      yearFilter.addEventListener("change", () => {
        currentPage = 1;
        if (sourceMode === "weekly" && !yearFilter.value && weeklyManifest?.latest_year) {
          yearFilter.value = String(weeklyManifest.latest_year);
        }
