(function () {
  ...
  function render(paper, publishedMonths) {
    ...
    root.innerHTML = `<div class="paper-detail-topline">
        ...
        ${paper.month_key && publishedMonths.has(paper.month_key) ? `<a href="archive/${escapeHtml(paper.month_key)}.html#w${escapeHtml(paper.number)}">所在月度合集</a>` : ""}
      </div>
      ...
      <div class="abstract-pair">
        ...
        <section class="abstract-block zh-block"><h3>中文摘要</h3><p>${escapeHtml(paper.zh_abstract || (paper.translation_state === "stale" ? "该论文的中文翻译正在更新中，当前版本暂不可用。" : "该论文暂无中文翻译。"))}</p></section>
      </div>
  ...
  Promise.all([
    ...
    loadJson("data/months.json").catch(() => []),
  ])
    .then(([items, months]) => {
